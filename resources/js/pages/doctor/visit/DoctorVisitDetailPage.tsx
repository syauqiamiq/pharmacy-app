...
const DoctorVisitDetailPage = (props: IDoctorVisitDetailPageProps) => {
    ...
    const { data, isLoading: visitLoading } = useGetVisitByID(props.visitId as string);
    ...
    return (
        <>
            ...
            >
                <Skeleton loading={visitLoading} active paragraph={{ rows: 4 }}>
                    <Space direction="vertical" size="large" className="mt-6 w-full">
                        <Tag color={getVisitStatusColor(data?.data.status || 'SCHEDULED')}>
                            {getVisitStatusText(data?.data.status || 'SCHEDULED')}
                        </Tag>
